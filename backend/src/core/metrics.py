...
tasks_total = _get_or_create_gauge(
    "tasks_total",
    "Total number of tasks in the system",
    ["status"],
)

task_operations_total = _get_or_create_counter(
    "task_operations",
    "Total number of task operations",
    ["operation", "status"],
)

task_operation_duration = _get_or_create_histogram(
...
attachments_total = _get_or_create_gauge(
    "attachments_total",
    "Total number of attachments in the system",
)

attachment_operations_total = _get_or_create_counter(
    "attachment_operations",
    "Total number of attachment operations",
    ["operation", "status"],
)

attachment_size_bytes = _get_or_create_histogram(
    "attachment_size_bytes",
    "Size of uploaded attachments in bytes",
    buckets=(1024, 10240, 102400, 1024000, 10240000, 52428800),
)

attachment_upload_duration = _get_or_create_histogram(
    "attachment_upload_duration_seconds",
    "Duration of attachment uploads in seconds",
...
reminders_processed_total = _get_or_create_counter(
